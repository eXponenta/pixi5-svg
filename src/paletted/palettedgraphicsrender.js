...
class PalettedShaderGenerator
{
    ...
    generateShader(maxTextures)
    {
        if (!this.programCache[maxTextures])
        {
			...
			const sampleSizeValues = new Float32Array(maxTextures * 2);
...
const vertex = `
	...
	void main(void){
		...
		if(aColor.r > 0.) {
			vStrokeColor = texture2D(uSamplers[textureId], pos + vec2(1. * size ,0.));
			vStrokeData.yz = texture2D(uSamplers[textureId], pos + vec2(2. * size ,0.)).xy;
		} else {
			vFillColor = texture2D(uSamplers[textureId], pos);
		}
